...
double fastzipf(double theta, long n, double zetan, prng_type * prng) {
    ...
    else val = 1 + (int64_t)(n * pow(eta*u - eta + 1., alpha));
...
{
    uint64_t high = static_cast<uint64_t>(nextUniformUnsignedLong());
    uint64_t low = static_cast<uint64_t>(nextUniformUnsignedLong());
    return (high << 32) | low;
}
...
Tools::PRGHurwitz::PRGHurwitz(int32_t min, int32_t max, double s, double a, Tools::Random* pRandom)
: m_min(min), m_max(max), m_s(s), m_a(a), m_pRandom(pRandom)
{
    initHurwitzLookupTable();
}

Tools::PRGHurwitz::~PRGHurwitz()
{
    delete[] m_pLookupTable;
}

void Tools::PRGHurwitz::initHurwitzLookupTable()
{
    uint32_t N = m_max - m_min;
    double Hns = 0.0;
    
    for (uint32_t k = 1; k < N; k++)
        Hns += 1.0 / std::pow(static_cast<double>(k + m_a), m_s);
    
    //fprintf(stderr, "Computed:%2.15f\n\n\n", Hns); 
    ...
    for (uint32_t k = 1; k < N; k++)
    {
        sum += 1.0 / std::pow(static_cast<double>(k + m_a), m_s);
        m_pLookupTable[k] = sum / Hns;
        //fprintf(stderr, "m_pLookupTable[%d] = %2.15f\n", k, m_pLookupTable[k]); 
    }
}

int32_t Tools::PRGHurwitz::nextHurwitzLong()
{
    uint32_t N = m_max - m_min;
    double dart = m_pRandom->nextUniformDouble();
    double* i = std::lower_bound(m_pLookupTable, m_pLookupTable + N, dart);
